...
import { Store, Receipt, Loader2 } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const [settings, setSettings] = useState({
    store_name: '', store_phone: '', store_address: '', tax_rate: '0', receipt_footer: '',
  });
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-3xl border border-gray-100 shadow-xs overflow-hidden flex flex-col md:flex-row min-h-[500px]">
            <div className="w-full md:w-64 bg-gray-50/50 border-b md:border-b-0 md:border-r border-gray-100 p-4 space-y-1.5 shrink-0 flex flex-row md:flex-col overflow-x-auto md:overflow-x-visible">
              <button
                ...
                className={`flex items-center gap-3 w-full text-left py-3 px-4 rounded-xl font-bold text-sm transition-all whitespace-nowrap md:whitespace-normal ${
                  activeTab === 'store' ? 'bg-gray-800 border border-gray-800 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-700'
                }`}
              >
                <Store className="w-5 h-5" /> Store Identity
              </button>
              ...
              <button
                ...
                className={`flex items-center gap-3 w-full text-left py-3 px-4 rounded-xl font-bold text-sm transition-all whitespace-nowrap md:whitespace-normal ${
                  activeTab === 'finance' ? 'bg-gray-800 border border-gray-800 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-700'
                }`}
              >
                <Receipt className="w-5 h-5" /> Financials & Receipts
              </button>
            </div>
            ...
            <div className="flex-1 p-6 md:p-8 flex flex-col justify-between">
                ...
                <div className="space-y-5 animate-fadeIn">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input type="text" required value={settings.store_name} onChange={e => handleInputChange('store_name', e.target.value)} className="w-full p-3.5 border border-gray-200 rounded-xl bg-gray-50/50 text-sm font-semibold text-gray-700 outline-hidden focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/10 transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5">Store Hotline Contact</label>
                      <input type="text" value={settings.store_phone} onChange={e => handleInputChange('store_phone', e.target.value)} className="w-full p-3.5 border border-gray-200 rounded-xl bg-gray-50/50 text-sm font-semibold text-gray-700 outline-hidden focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/10 transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5">Physical Store Address</label>
                      <textarea rows={3} value={settings.store_address} onChange={e => handleInputChange('store_address', e.target.value)} className="w-full p-3.5 border border-gray-200 rounded-xl bg-gray-50/50 text-sm font-semibold text-gray-700 outline-hidden focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/10 transition-all resize-none" />
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="relative">
                        <input type="number" step="0.01" required min="0" max="100" value={settings.tax_rate} onChange={e => handleInputChange('tax_rate', e.target.value)} className="w-full p-3.5 pr-10 border border-gray-200 rounded-xl bg-gray-50/50 text-sm font-bold text-gray-700 outline-hidden focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/10 transition-all" />
                    ...
                    <div>
                      ...
                      <input type="text" value={settings.receipt_footer} onChange={e => handleInputChange('receipt_footer', e.target.value)} className="w-full p-3.5 border border-gray-200 rounded-xl bg-gray-50/50 text-sm font-semibold text-gray-700 outline-hidden focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/10 transition-all" />
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="mt-8 pt-4 border-t border-gray-100 flex justify-end shrink-0">
                <button type="submit" disabled={isSaving} className="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white font-bold px-8 py-3.5 rounded-xl shadow-md transition-all active:scale-98 text-sm disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed">
